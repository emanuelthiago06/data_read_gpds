...
regex_signal_name = r"^Results_alt/(\w+)/(?P<numero>\d+_hr)"
cont =0
contSignal = 1
...
for j in loops:
    dados = pd.DataFrame(columns=('signalName', 'cycle',
                              'd1_X', 'd2_X', 'd_dif_X', 'sim_X', 'd_pond_X',
                              'd1_Y', 'd2_Y', 'd_dif_Y', 'sim_Y', 'd_pond_Y',
                              'd1_Z', 'd2_Z', 'd_dif_Z', 'sim_Z', 'd_pond_Z'))
    for root,dir,files in os.walk(f"Results_alt/{j}",topdown=False):
        if dir:
            print(dir)
            break
        total_signal+=1
    ...
    for root,dir,files in os.walk(f"Results_alt/{j}",topdown=False):
        ...
        cycle = "not implemented, fix later"

        dados.loc[cont] = [signalName, cycle, cm_fixo1_X, cm_fixo2_X, dif_fixo_X, sim_fixo_X, d_pond_fixo_X,

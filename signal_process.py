...
regex_signal_name = r"^Results_alt2/(\w+)/(?P<classe>\d)/(?P<numero>\d+_hr)/(?P<ciclo>\d+)"
regex_dir_stop = r"^(?P<numero>\d+_hr)"
cont =0
contSignal = 1
dir_exe = ['0','1']
...
n = 0
for j in loops:
    dados = pd.DataFrame(columns=('signalName', 'cycle','classe','split',
                              'd1_X', 'd2_X', 'd_dif_X', 'sim_X', 'd_pond_X',
                              'd1_Y', 'd2_Y', 'd_dif_Y', 'sim_Y', 'd_pond_Y',
                              'd1_Z', 'd2_Z', 'd_dif_Z', 'sim_Z', 'd_pond_Z'))
    for root,dir,files in os.walk(f"Results_alt2/{j}",topdown=False):
        try:
            tr = root+'/'+files[1]
            total_signal+=1
        except:
    ...
    for root,dir,files in os.walk(f"Results_alt2/{j}",topdown=False):
        try:
            ...
            dados.loc[cont] = [signalName, cycle, classe, j, cm_fixo1_X, cm_fixo2_X, dif_fixo_X, sim_fixo_X, d_pond_fixo_X,
    ...
    contSignal = 0
    total_signal = 0
    dados.to_csv(f'dados_{j}_ptb.csv', mode='w')
    del dados
# Finalização do 
print(n)
end = timer()
